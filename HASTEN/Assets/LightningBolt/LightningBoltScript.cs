...
namespace DigitalRuby.LightningBolt
{
    ...
    public class LightningBoltScript : MonoBehaviour
    {
        ...
        {
            if (other.gameObject.tag.Equals("Enemy") && EnemyID == -1)
            {
        ...
        {
            if (other.gameObject.GetInstanceID() == EnemyID)
            {
        ...
        IEnumerator Attack(Collider other)
        {
            while(EnemyID != -1)
            {
                other.gameObject.GetComponent<CUnit>().getDamage(1);
                if (other.GetComponent<CUnit>().HP <= 0) 
                {
                    OnTriggerExit(other);
                    break;
                }
